...
ignore_categories = {
    "Obsidian": ["Bug reports"],
    "Level1Techs": ["GPU"],
}


def get_discourse_posts(forum_url: str, ignored_categories: list = None) -> pd:
    ...
    posts_df["Headline"] = [f"{str(x)} [{z}]| href = {y}" for x, y, z in
                            zip(posts_df['Topic'], posts_df['Topic_url'], posts_df['Category'])]

    # Check if there are categories to ignore
    if ignored_categories:
        posts_df = posts_df[~posts_df['Category'].isin(ignored_categories)]
...
for forum_title, forum_url in forums.items():
    ignored_categories = ignore_categories.get(forum_title)
    posts_df = get_discourse_posts(forum_url, ignored_categories)
